...
$("#modal-categorias").dialog("open");
	  ...
	  $("#busca-categorias").keyup(function(){
		...
		$.ajax({
			type: "post",
			url: "/sistemacategorias/live_search",
			cache: false,				
				...
				if(obj.length>0){
					try{
						...
						$.each(obj, function(i,val){
						    ...
						    items.push($('<tr/>').prepend("<td>" + val.id + "</td><td>" + val.nome + "</td><td>" + val.sistemamodulo_nome + "</td><td><button class=\"close\" id=\"" + val.id + "\" nome=\"" + val.nome + "\">selecionar</button></td>"));
						});	
						$('#resultado-categorias').append.apply($('#resultado-categorias'), items);

			$(".close").click(function () {

				var sistemacategoria_id = $(this).attr('id');
				var nome = $(this).attr('nome');

                                if(sistemacategoria_id){
					$("#sistemacategoria_id").val(sistemacategoria_id);
